...
**最終更新日**: 2025-12-01 (Phase 2.10 Part 2: tk_wai_sem_u実装完了)

---

## 🚧 現在の作業: Phase 2.10 タイムアウト機能実装 (2025-12-01)

### 実装方針: オプションB（最小実装アプローチ）

**決定事項**:
- まず `tk_wai_sem_u` を完全に実装・テスト
- 実装パターンを確立してから残りのシステムコールに適用
- 段階的アプローチで確実な実装を目指す

### Phase 2.10 - Part 1: インフラ構築 ✅ 完了

**コミットID**: `300ea7e`

**完了項目**:
1. **型定義の追加** (`types.h`)
   - `SYSTIM` 型（64ビット、ミリ秒単位のシステム時刻）
   - `TMO` 型（タイムアウト値、ミリ秒単位）
   - `TMO_POL` (0) - ポーリング（待たない）
   - `TMO_FEVR` (-1) - 無限待ち
   - `E_TMOUT` (-11) - タイムアウトエラーコード

2. **TCB拡張** (`kernel_main.c`)
   - `wait_timeout` フィールド追加（SYSTIM型）
   - 絶対タイムアウト時刻を保存（0 = タイムアウトなし）

### Phase 2.10 - Part 2: tk_wai_sem_u実装 ✅ 完了

**完了項目**:
1. **システムコール番号追加**
   - `SVC_WAI_SEM_U` (#21) を定義

2. **svc_wai_sem_u()関数実装** (`kernel_main.c:707-787`)
   - TMO_POL (ポーリング) 対応: 即座にE_TIMOUTまたはE_OKを返す
   - TMO_FEVR (無限待ち) 対応: wait_timeout=0でタイムアウトなし
   - 有限タイムアウト: wait_timeout = timer_tick_count + tmo
   - wait_regs保存: タイムアウト時の返値設定用

3. **tk_wai_sem_u()ラッパー実装** (`kernel_main.c:1910-1921`)
   - インラインアセンブリで2パラメータ渡し
   - x0: semid, x1: tmo
   - SVC #21 呼び出し

4. **check_timeouts()関数実装** (`kernel_main.c:2660-2691`)
   - 全タスクをスキャンしてタイムアウトチェック
   - タイムアウト発生時:
     - 待ちキューから削除
     - 返値をE_TIMOUTに設定
     - タスク状態をREADYに変更

5. **タイマー割り込みへの統合** (`kernel_main.c:2696-2710`)
   - timer_handler()からcheck_timeouts()を呼び出し
   - 毎ティック（1ms）でタイムアウトチェック実行

6. **テストコード作成** (`kernel_main.c:2258-2523`)
   - 包括的なテストスイート（6つのテストケース）
   - Task1: タイムアウトテスト実行
     - Test 1: 短いタイムアウト（50ms）でセマフォ保持中
     - Test 2: ポーリング（TMO_POL）でセマフォ保持中
     - Test 3: 長いタイムアウトでセマフォ利用可能
     - Test 4: ポーリングでセマフォ保持
     - Test 5: ポーリングでセマフォ利用可能
     - Test 6: 無限待ち（TMO_FEVR）でセマフォ利用可能
   - Task2: セマフォ保持タスク
     - 起動時にセマフォを取得
     - 150ms保持（Test 1, 2がタイムアウト検証可能に）
     - 解放してTask1の残りテストを成功させる

7. **tk_dly_tsk()の修正** (`kernel_main.c:525-556`)
   - 問題: 元実装はビジーウェイトでCPUを占有
   - 修正: タイムアウト機構を使用してタスクを適切にWAIT状態に
   - wait_obj = NULL でタスク遅延を示す
   - タイムアウト時にE_OKを返す（正常完了）

8. **check_timeouts()の拡張** (`kernel_main.c:2795-2858`)
   - オブジェクト待ち: wait_obj != NULL → E_TIMOUTを返す
   - タスク遅延: wait_obj == NULL → E_OKを返す
   - 待ちキューから安全に削除
   - タスク状態をREADYに変更

9. **timer_handler()の改善** (`kernel_main.c:2863-2877`)
   - 問題: check_timeouts()後に即座にスケジュールされず、最大100ms遅延
   - 修正: check_timeouts()の直後にschedule()を呼び出し
   - 効果: タイムアウト満了タスクが即座に実行される
   - テストの正確なタイミング制御が可能に

### Phase 2.10 - Part 3以降: 次のステップ

**優先順位**:
1. **残りのタイムアウト付きシステムコール**
   - tk_loc_mtx_u（ミューテックス）
   - tk_wai_flg_u（イベントフラグ）
   - tk_snd_mbf_u, tk_rcv_mbf_u（メッセージバッファ）
   - tk_rcv_mbx_u（メールボックス）

3. **Phase 2.11: タスクサスペンド機能**

### 技術的考慮事項

**タイムアウト実装の課題**:
- タイマー割り込みでの効率的なタイムアウトチェック
- 各オブジェクトタイプの待ちキューからの安全な削除
- WAIT状態からの正しいタスク復帰処理
- エラーコード（E_TMOUT）の適切な返却

**選択した最小実装アプローチの利点**:
- セマフォでパターンを確立後、他のオブジェクトに展開
- 段階的テストで品質を担保
- 問題の早期発見と修正が容易
