...
/*
 * Set task register contents (AArch64)
 */
...
/*
 * Get task register contents (AArch64)
 */
LOCAL void get_reg( TCB *tcb, T_REGS *regs, T_EIT *eit, T_CREGS *cregs )
{
	...
	if ( eit != NULL ) {
		...
		if ( tcb->tskctxb.svc_ssp == NULL ) {
			...
			/* The value immediately before the T-Kernel system call (SVC) during
			   the call is made. */
			eit->taskmode = *(tcb->tskctxb.svc_ssp - 1);
		}
	}
...
/*
 * Set task coprocessor register (FPU/SIMD for AArch64)
 */
SYSCALL ER _tk_set_cpr( ID tskid, INT copno, CONST T_COPREGS *pk_copregs )
{
	...
	if ( tcb->state == TS_NONEXIST ) {
		...
		/* FPU/SIMD register operations would go here */
	}
	END_CRITICAL_SECTION;
...
/*
 * Get task coprocessor register (FPU/SIMD for AArch64)
 */
