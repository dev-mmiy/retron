...
/*
 * DAIF bits for interrupt control
 */
#define DAIF_I_BIT	(1 << 7)	/* IRQ mask bit */
#define DAIF_F_BIT	(1 << 6)	/* FIQ mask bit */
#define DAIF_IF_BITS	(DAIF_I_BIT | DAIF_F_BIT)

/*
 * Check if interrupts are disabled
 */
Inline BOOL isDI( UINT daif )
{
 ...
 */
#define BEGIN_CRITICAL_SECTION	{ UINT _daif_ = disint();
#define END_CRITICAL_SECTION	if ( !isDI(_daif_)			\
				  && ctxtsk != schedtsk			\
				  && !isTaskIndependent()		\
				  && !dispatch_disabled ) {		\
					dispatch();			\
 ...
 */
#define BEGIN_DISABLE_INTERRUPT	{ UINT _daif_ = disint();
#define END_DISABLE_INTERRUPT	enaint(_daif_); }
...
/*
 * Task context block for AArch64
 */
typedef struct {
	...
	UW	spsr;		/* Saved Program Status Register */
} CTXB;
