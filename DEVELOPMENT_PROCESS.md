...
作業内容が反映されているかを素早く確認するため、以下のチェックを標準化します。

- `git status -sb` で作業ツリーがクリーンか確認し、未コミット変更を把握する。
- `git log -1 --stat` で直近コミットに対象変更が含まれるか確認する。
- 必要に応じて `git show` で差分を確認し、コメント対応やレビュー指摘が反映されているかを確認する。
- 上記を満たしたら `git push` を実行し、リモートにも確実に反映する。
