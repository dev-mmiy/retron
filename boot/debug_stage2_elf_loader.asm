...
load_kernel:
    ...
    ; ELFカーネルを読み込み（複数セクタ）
    mov ah, 0x02        ; 読み込み機能
    mov al, 20          ; 20セクタ読み込み（10KB）
    mov ch, 0           ; シリンダー
    mov cl, 11          ; セクタ（カーネルは11セクタ目から）
    mov dh, 0           ; ヘッド
...
; メッセージ定義
boot_msg db 'Retron OS Stage2 ELF Loader v1.0', 13, 10, 0
...
debug_load_addr_msg db 'DEBUG: LoadAddr: 0x', 0
...
ph_info_msg db 'PH Offset: 0x', 0
ph_count_msg db 'PH Count: 0x', 0
