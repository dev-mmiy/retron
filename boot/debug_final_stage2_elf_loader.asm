...
load_kernel:
    ...
    ; ELFカーネルを読み込み（修正されたセクタ番号6）
    mov ah, 0x02        ; 読み込み機能
    mov al, 21          ; 21セクタ読み込み（10.5KB）
    mov ch, 0           ; シリンダー
    mov cl, 6           ; セクタ（カーネルは6セクタ目から）
    mov dh, 0           ; ヘッド
...
load_single_segment:
    ...
    add ebx, 3072             ; カーネル開始オフセット（6セクタ * 512バイト）
...
; メッセージ定義
boot_msg db 'Retron OS Debug Final Stage2 ELF Loader v1.0', 13, 10, 0
